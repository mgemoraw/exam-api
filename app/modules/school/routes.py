...
@school_router.get("/universities/{uid}", response_model=UniversityResponse)
async def get_university(uid:str, db: Session = Depends(get_db)):
    service = UniversityService(db)
...
@school_router.get("/universities/name-or-code", response_model=UniversityResponse)
async def get_university_by_code_or_name(name:str=None, code:str = None, db: Session = Depends(get_db)):
    service = UniversityService(db)

    university = service.get_by_name_or_code(name=name, code=code)
...
async def create_university(university: UniversityCreateRequest, address: AddressCreateRequest=None, db: Session = Depends(get_db)):
    ...
    service = UniversityService(db)
    return service.create_university(university, address)
    # try:
...
@school_router.put("/universities/{university_id}")
async def update_university(university_id: uuid.UUID, university_data: UniversityUpdateRequest, db: Session = Depends(get_db)):
    service = UniversityService(db)
    service.update_university(str(university_id), university_data)
...
@school_router.get("/programs", response_model=list[ProgramResponse])
